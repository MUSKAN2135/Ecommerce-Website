...
export default function Shopproducts() {
    ...
    const totalPages = Math.ceil(showProducts.length / productsPerPage);// ceil used to rounds up pages 
    // Page numbers list
    const pageNumbers = Array.from({ length: totalPages }, (_, i) => i + 1); //=> this means map each index with pagenmbr that starts from 1
                        ...
                        <div>
                                ...
                                {currentProducts.map((p) => (
                                    <div key={p.id} className="bg-white rounded-xl shadow-md hover:shadow-lg transition-shadow duration-300 p-4 flex flex-col justify-between">
                                        ...
                                        <div className="flex items-center py-2">
                                            <Link to="/cart" className="bg-[#0097b2] py-2 px-3 rounded-md hover:text-black text-white hover:border hover:border-[#0097b2] hover:bg-transparent transition duration-150"> Add to Cart
                                            </Link>
                            ...
                            <div className="flex justify-center mt-8 gap-2">
                                {pageNumbers.map((page) => (
                                    <button key={page} onClick={() => setCurrentPage(page)} className={`px-4 py-2 text-sm ${currentPage === page ? "bg-[#0097b2] text-white" : "text-gray-700 border-gray-300 hover:bg-gray-100"}`} >
                                        {page}
                                    </button>
                                ))}
                            </div>
